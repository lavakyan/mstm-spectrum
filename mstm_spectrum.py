...
class Profiler(object):
    """
    Class for benchmarking from http://onesteptospace.blogspot.pt/2013/01/python.html
    Usage:
    >>> with Profiler() as p:
...
class SPR(object):
    ...
    def set_spheres(self, spheres):
        ...
        self.paramDict['number_spheres'] = len(self.spheres)
    ...
    def set_incident_field(self, fixed=False, azimuth_angle=0.0, polar_angle=0.0, polarization_angle=0.0):
        """
            ...
            polarization_angle: float (degrees)
                ...
                0 - X-polarized, 90 - Y-polarized (if azimuth and polar angles are zero).
        """
        if not fixed :
