...
import import_datebase_support 
from tkinter import filedialog, messagebox, Menu
import zipfile

global extract_dir
extract_dir = None      # Глобальная переменная для директории распаковки
valid_files = []        # Глобальный список для хранения проверенных файлов
        ...
        try:
            ...
            for item in items:
                ...
                if os.path.isdir(item_path):
                    ...
                    try:
                        mat = import_datebase_support.import_from_file_plot(item_path)
                        mat.get_n(250)
                        mat.get_k(250)
                        mat.get_n(1000)
                        mat.get_k(1000)
...
def btLoadData(root, menu_button):
    ...
    if extract_dir is None:
        ...
        def select_yml_file(file_path):
            mat = import_datebase_support.import_from_file_plot(file_path)
            key = gen_mat_key()
            add_material(key, mat)
            update_materials_tree()
