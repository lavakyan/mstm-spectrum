...
class SPR(object):
    ...
    @property
    def environment_material(self):
        return self._environment_material

    @environment_material.setter
    def environment_material(self, material):
        ''' Setup environent material, where spheres are placed.
            Accepts string, float, complex or Materials instance
            inputs
        '''
    ...
    def _write_input(self, tmpdir):
        ...
        for wl in self.wavelengths:
            ...
            outFID.write('  %f\n' % self._environment_material.get_n(wl))
            outFID.write('medium_imag_ref_index\n')
            outFID.write('  %f\n' % self._environment_material.get_k(wl))
    ...
    def _execute_mstm(self, tmpdir):
        ''' run MSTM binary in tmpdir
            Windows / Linux
    ...
    def simulate(self, outfn=None):
        ...
        The inpuit parameters are read from object
        dictionary `paramDict`.
        Routine will prepare input file `scriptParams.inp`
        ...
        outfn -- filename to store results. None - no file used (a bit faster)
        '''
