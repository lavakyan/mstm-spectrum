...
try:
    from mstm_studio.film_exctinction import gold_film_ex  # for gold film background
except:
    pass
    ...
    def simulate(self, outfn=None):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if sys.platform == 'win32':
                ...
                subprocess.call('%s scriptParams.inp > NUL' % self.command, shell=True, startupinfo=si, cwd=tmpdir)
            else:
                subprocess.call('%s scriptParams.inp > /dev/null' % self.command, shell=True, cwd=tmpdir)
            ...
            if self.paramDict['fixed_or_random_orientation'] == 0:  # fixed orientation
                ...
                self.scattering_ort = []
                for l in self.wavelengths:
                    inFID = open(os.path.join(tmpdir, 'mstm_l%.0f.out' % (l*1000)), 'r')
                    while True:
                        line = inFID.readline()
                    ...
                    inFID.close()
                    os.remove(os.path.join(tmpdir, 'mstm_l%.0f.out' % (l*1000)))  # cleanup
            ...
            # tmpdir.cleanup()  # clear temporary directory - will be done automatically becouse of 'with'
...
if __name__== '__main__':
    ...
    with Profiler() as p:
        ...
        spheres = ExplicitSpheres(2, [0,0,0,10,0,0,0,12], mat_filename=['nk/etaGold.txt','nk/etaSilver.txt'])
